classdef OSCellMovie < Core.OSLocMovie
    ...
    methods
        ...
        function obj = OSCellMovie(ext,info,varargin)
            
            obj  = obj@Core.OSLocMovie(ext,info,varargin{1});
            
        end
        
        function [finalBW] = segmentLamina(obj,sensitivity,clearBorder)
            ...
            if sum(idx)~=1
                ...
                threshold = adaptthresh(uint16(IMs),sensitivity,'neigh',neigh);
                gBW = imbinarize(uint16(IMs),threshold);
             %   gBW = imbinarize(uint16(IMs));
                gBW = bwareaopen(gBW,round(0.03*numel(gBW(:,:,1))));
                if clearBorder
                    for i = 1:size(gBW,3)
                        gBW(:,:,i) = imclearborder(gBW(:,:,i));
                    end
                end    
                se = strel('disk',10);
                gBW = imclose(gBW,se);
                ...
                for i =1:size(gBW,3)
                    BWdata = regionprops(gBW(:,:,i),{'Area','Perimeter', 'pixelIDXList'});
                    tmpBW  = zeros(size(gBW(:,:,i)));
                    [~,maxArea] = max([BWdata.Perimeter]);
                                  
                    tmpBW(BWdata(maxArea).PixelIdxList) = 1;
        ...
        function [finalBW] = segmentNUP(obj,sensitivity,clearBorder)
        ...
        function [finalBW] = segmentRedLipid(obj)
            ...
            if sum(idx)~=1
                ...
                IMs = imgaussfilt3(data, sigma);
                disp('DONE with filtering ------------')
                threshold = adaptthresh(uint16(IMs),0.5,'neigh',[301 301 3]);
                gBW = imbinarize(uint16(IMs),threshold);
                gBW = bwareaopen(gBW,10000);
%                 se = strel('disk',10);
%                 gBW = imclose(gBW,se);
...
%                     end
        ...
        function showAllMembranes(obj)
            disp('Plotting all membranes')
            figure
            hold on
            leg = cell(1,4);
            for i = 1:length(obj.dataType)
                currentMem = obj.dataType{i};
               
                try
                    [~,contour,fitMembrane] = getMembrane(obj,currentMem);
                catch e
                ...
                if ~exist('e')
                    ...
                    if ~isempty(fitMembrane)
                        plot(fitMembrane{idx}(:,2),fitMembrane{idx}(:,1),'Linewidth',1);
                    end
                ...
                clear e contour fitMembrane
            end
            leg(cellfun(@isempty,leg)) = [];
            legend(leg)
        end
    ...
    methods (Access = private)
        ...
        function [idx,xVec,yVec] = getLinePixel(obj,linEq,dim)
            m = linEq.m;
            b = linEq.b;
            x = linEq.x;
            y = linEq.y;
            ...
            [xVec,yVec] = obj.fixVec(xVec,yVec,dim);
